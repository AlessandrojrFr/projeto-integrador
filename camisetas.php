<!doctype html>
<html lang="pt-br">

<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Camisetas Nike</title>
  <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0-alpha2/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-aFq/bzH65dt+w6FI2ooMVUpc+21e0SRygnTpmBvdBgSdnuTN7QbdgL+OapgHtvPp" crossorigin="anonymous">
  <link rel="stylesheet" href="nikezinho.css">
</head>

<body>
  <nav class="navbar navbar-expand-lg bg-body-tertiary">
    <div class="container-fluid">
      <a class="navbar-brand" href="login.php"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="18" viewBox="0 0 20 18" fill="none">
          <path d="M10.84 1.14V1.08C10.84 0.8 10.93 0.55 11.11 0.33C11.29 0.11 11.52 0 11.8 0C12.08 0 12.33 0.0900001 12.55 0.27C12.77 0.45 12.89 0.69 12.91 0.99C12.93 1.29 12.84 1.54 12.64 1.74C12.44 1.94 12.2 2.04 11.92 2.04L11.74 2.1L11.8 2.19L11.74 2.52L11.62 3.48C11.66 3.52 11.68 3.56 11.68 3.6L11.56 4.2C11.52 4.28 11.48 4.34 11.44 4.38L11.38 4.62C11.3 4.94 11.23 5.2 11.17 5.4C11.11 5.6 11.06 5.76 11.02 5.88V6C10.98 6.2 10.95 6.36 10.93 6.48C10.91 6.6 10.84 6.8 10.72 7.08C10.64 7.24 10.64 7.5 10.72 7.86L10.78 7.92C10.78 8.04 10.83 8.18 10.93 8.34C11.03 8.5 11.08 8.64 11.08 8.76C11.12 9.56 11.06 10.24 10.9 10.8L11.02 11.16C11.5 11.4 11.64 11.66 11.44 11.94L11.74 12.06C12.22 12.3 12.56 12.51 12.76 12.69C12.96 12.87 13.16 13.06 13.36 13.26C13.52 13.3 13.64 13.36 13.72 13.44L13.9 13.5C14.74 14.06 15.58 14.74 16.42 15.54L16.66 15.72V15.78L16.6 15.9L16.78 16.02H16.84C16.96 16.1 17.04 16.14 17.08 16.14H17.14C17.18 16.14 17.22 16.12 17.26 16.08L17.38 16.02C17.46 15.94 17.54 15.88 17.62 15.84H17.86C17.9 15.84 17.9 15.86 17.86 15.9L17.62 16.02L17.32 16.38H17.74L18.16 16.44C18.28 16.4 18.38 16.36 18.46 16.32L18.76 16.14C18.88 16.1 19 16.14 19.12 16.26H19.18C19.22 16.3 19.2 16.36 19.12 16.44L18.4 17.1C18.2 17.26 18.02 17.36 17.86 17.4L17.08 17.94C17.04 17.98 17 17.98 16.96 17.94L16.78 17.7L16.66 17.46L16.51 17.25L16.33 17.01L16.18 16.83L16.06 16.68L15.82 16.5C15.74 16.5 15.66 16.48 15.58 16.44L14.8 15.84C14.68 15.84 14.54 15.78 14.38 15.66C13.78 15.14 13.38 14.82 13.18 14.7L12.82 14.46L12.1 14.34C11.86 14.3 11.54 14.18 11.14 13.98L10.66 13.74C10.34 13.58 10.12 13.5 10 13.5L9.76 13.38C9.6 13.34 9.48 13.3 9.4 13.26L9.28 13.14C9.2 13.14 9.12 13.16 9.04 13.2L7.84 13.8L6.34 14.46C5.94 14.78 5.56 15.04 5.2 15.24L4.54 15.54L3.52 16.26C3.44 16.34 3.36 16.34 3.28 16.26L3.16 16.38C3.04 16.42 2.96 16.44 2.92 16.44L2.68 16.56V16.68H2.56L2.44 16.86C2.36 17.02 2.28 17.11 2.2 17.13C2.12 17.15 2.07 17.18 2.05 17.22C2.03 17.26 2.01 17.29 1.99 17.31L1.87 17.43L1.69 17.55L1.42 17.52L1.06 17.58L0.82 17.64C0.62 17.68 0.46 17.65 0.34 17.55C0.22 17.45 0.12 17.3 0.04 17.1C-0.04 16.98 0 16.88 0.16 16.8L0.22 16.74C0.26 16.7 0.32 16.68 0.4 16.68H0.76L1.24 16.5L1.48 16.44C1.48 16.36 1.5 16.3 1.54 16.26C1.58 16.22 1.64 16.2 1.72 16.2V16.14C1.68 16.06 1.66 15.98 1.66 15.9C1.62 15.82 1.61 15.76 1.63 15.72C1.65 15.68 1.67 15.66 1.69 15.66H1.72L1.78 15.84C1.82 16.04 1.88 16.1 1.96 16.02L2.02 15.9C2.06 15.82 2.12 15.78 2.2 15.78L2.32 15.9L2.44 15.78L2.38 15.72C2.38 15.68 2.4 15.66 2.44 15.66L2.68 15.42C2.88 15.18 3.12 14.96 3.4 14.76C3.96 14.32 4.56 13.98 5.2 13.74C5.4 13.54 5.62 13.44 5.86 13.44C6.02 13.16 6.26 12.86 6.58 12.54C6.86 12.34 7.06 12.2 7.18 12.12C7.26 11.96 7.36 11.88 7.48 11.88H7.54L7.66 11.76C7.74 11.72 7.8 11.68 7.84 11.64V11.34C7.84 11.14 7.86 10.98 7.9 10.86C7.94 10.74 8.04 10.68 8.2 10.68L8.44 10.44C8.36 10.28 8.32 10.1 8.32 9.9H8.26C8.18 9.78 8.14 9.66 8.14 9.54C8.02 9.34 7.94 9.18 7.9 9.06H7.72C7.68 9.18 7.58 9.26 7.42 9.3L7.36 9.42C7.2 9.7 7.06 9.91 6.94 10.05C6.82 10.19 6.6 10.36 6.28 10.56C6.08 10.76 5.94 10.98 5.86 11.22C5.82 11.34 5.82 11.46 5.86 11.58L5.8 11.7H5.86C5.86 11.78 5.88 11.82 5.92 11.82H5.98C6.06 11.86 6.1 11.91 6.1 11.97C6.1 12.03 6.04 12.05 5.92 12.03C5.8 12.01 5.71 11.97 5.65 11.91C5.59 11.85 5.54 11.82 5.5 11.82L5.32 12C5.24 12.12 5.16 12.17 5.08 12.15C5 12.13 4.98 12.1 5.02 12.06L5.08 12C5.12 11.92 5.12 11.88 5.08 11.88L4.6 12C4.56 12.04 4.51 12.04 4.45 12C4.39 11.96 4.4 11.92 4.48 11.88L4.78 11.76C4.78 11.72 4.76 11.7 4.72 11.7C4.56 11.78 4.4 11.8 4.24 11.76L3.94 11.7L3.88 11.64C3.88 11.6 3.9 11.58 3.94 11.58C4.1 11.62 4.3 11.6 4.54 11.52L4.96 11.34L5.44 10.8L5.5 10.68C5.66 10.36 5.83 10.07 6.01 9.81C6.19 9.55 6.38 9.34 6.58 9.18L6.64 8.94C6.72 8.78 6.8 8.64 6.88 8.52C6.96 8.4 7.06 8.24 7.18 8.04L7.36 7.8C7.52 7.56 7.7 7.44 7.9 7.44L8.08 7.26C8.12 7.22 8.14 7.16 8.14 7.08L8.26 6.96L8.2 6.9C7.96 6.7 7.84 6.52 7.84 6.36C7.8 6.08 7.89 5.84 8.11 5.64C8.33 5.44 8.55 5.35 8.77 5.37C8.99 5.39 9.16 5.46 9.28 5.58L9.4 5.7C9.44 5.7 9.46 5.72 9.46 5.76L9.7 5.88V6.06C9.78 6.14 9.82 6.2 9.82 6.24C9.9 6.12 10.02 5.98 10.18 5.82L10.48 4.86C10.48 4.7 10.52 4.54 10.6 4.38L10.72 4.2V4.08L10.9 3.36H11.02L11.2 2.64C11.24 2.52 11.24 2.38 11.2 2.22L10.96 1.62L10.84 1.14Z" fill="black" />
        </svg></a>
      <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
        <span class="navbar-toggler-icon"></span>
      </button>
      <div class="collapse navbar-collapse" id="navbarSupportedContent">
        <ul class="navbar-nav me-auto mb-2 mb-lg-0">
          <li class="nav-item">
            <a class="nav-link active" aria-current="page" href="nikezinho.php">INÍCIO</a>
          </li>
          <li class="nav-item">
            <a class="nav-link" href="nikezinho.php"></a>
          </li>
          <li class="nav-item dropdown">
            <a class="nav-link dropdown-toggle" href="nikezinho.php" role="button" data-bs-toggle="dropdown" aria-expanded="false">
            CAMISETAS
            </a>
          <li class="nav-item dropdown">
            <a class="nav-link dropdown-toggle" href="nikezinho.php" role="button" data-bs-toggle="dropdown" aria-expanded="false">
           Feminino
            </a>
          </li>
          <li class="nav-item">
            <a class="nav-link active" aria-current="page" href="login.php">CADASTRAR</a>
          </li>

        </ul>
        <form class="d-flex" role="search">
          <input class="form-control me-2" type="search" placeholder="Fale Conosco" aria-label="Search">
          <button class="btn btn-outline-success" type="submit">AJUDA</button>
        </form>
      </div>
    </div>
  </nav>
  <br>

  <div id="camiseta">
    <img src="img-projeto/tout-corinthians-13-04-desk.avif" alt="...">
  </div>
  <br>


  <div class="container">
    <div class="row">
      <div class="col">
        <div class="card" style="width: 18rem;">
          <img src="img-projeto/timao.avif" class="card-img-top" alt="...">
          <div class="card-body">
            <h5 class="card-title">Camiseta Nike Dri-FIT Feminina</h5>
            <h5 class="card-title">Treino & Academia</h5>
            <p>Feminina</p>
            <p>Dri-FIT</p>
             <h2>R$ 49,99
            R$ 149,99
            67% off
            ou 1x de R$ 49,99</h2>
          </div>
        </div>
      </div>

      <div class="col">
        <div class="card" style="width: 18rem;">
          <img src="img-projeto/brasil.avif" class="card-img-top" alt="...">
          <div class="card-body">
            <h5 class="card-title">Regata Nike Dri-FIT Feminina</h5>
            <h5 class="card-title">Treino & Academia</h5>
            <p>Feminina</p>
            <p>Dri-FIT</p>
            <h2>R$ 79,99
              R$ 149,99
              47% off
            ou 1x de R$ 79,99</h2>
          </div>
        </div>
      </div>

      <div class="col">
        <div class="card" style="width: 18rem;">
          <img src="img-projeto/BARCELONA1.jpg" class="card-img-top" alt="...">
          <div class="card-body">
            <h5 class="card-title">Regata Nike Dri-FIT One Feminina</h5>
            <h5>Treino & Academia</h5>
            <p class="card-text">Feminina</p>
            <p>Dri-FIT</p>
            <h2>R$ 169,99
            R$ 229,99
           26% off
          ou 1x de R$ 169,99</h2>
          
          </div>
        </div>
      </div>

    </div>
  </div>
  <br>
  <br>
  <br>

  <h1>Lançamentos</h1>
  <div id="lançamento">
    <img src="img-projeto/tout-gamma-force-05.06-desk-v2.avif" alt="...">
</div>


<div class="col">
        <div class="card" style="width: 18rem;">
          <img src="img-projeto/brasil.avif" class="card-img-top" alt="...">
          <div class="card-body">
            <h5 class="card-title">Camiseta Nike Dri-FIT Swoosh Fly Feminina</h5>
            <h5 class="card-title">Treino & Academia</h5>
            <p>Feminina</p>
            <p>Dri-FIT</p>
            <h2>R$ 149,99
              R$ 229,99
              35% off
            ou 1x de R$ 149,99</h2>
          </div>
        </div>
      </div>
    

      <div class="col">
        <div class="card" style="width: 18rem;">
          <img src="img-projeto/brasil.avif" class="card-img-top" alt="...">
          <div class="card-body">
            <h5 class="card-title">Camiseta Nike Dri-FIT One Feminina</h5>
            <h5 class="card-title">Treino & Academia</h5>
            <p>Feminina</p>
            <p>Dri-FIT</p>
            <h2>R$ 89,99
            R$ 129,99
              31% off
            ou 1x de R$ 89,99</h2>
          </div>
        </div>
      </div>
    

      <div class="col">
        <div class="card" style="width: 18rem;">
          <img src="img-projeto/brasil.avif" class="card-img-top" alt="...">
          <div class="card-body">
            <h5 class="card-title">Camiseta Nike Pro Dri-FIT Feminina</h5>
            <h5 class="card-title">Treino & Academia</h5>
            <p>Feminina</p>
            <p>Dri-FIT</p>
            <h2>R$ 129,99
              R$ 249,99
              48% off
            ou 1x de R$ 129,99</h2>
          </div>
        </div>
      </div>

      <div class="col">
        <div class="card" style="width: 18rem;">
          <img src="img-projeto/brasil.avif" class="card-img-top" alt="...">
          <div class="card-body">
            <h5 class="card-title">Camiseta Nike Dri-FIT Swoosh Fly Feminina</h5>
            <h5 class="card-title">Treino & Academia</h5>
            <p>Feminina</p>
            <p>Dri-FIT</p>
            <h2>R$ 149,99
            R$ 229,99
            35% off
          ou 1x de R$ 149,99</h2>
          </div>
        </div>
      </div>


 <div class="col">
        <div class="card" style="width: 18rem;">
          <img src="img-projeto/brasil.avif" class="card-img-top" alt="...">
          <div class="card-body">
            <h5 class="card-title">Camiseta Nike Pro Dri-FIT Feminina</h5>
            <h5 class="card-title">Treino & Academia</h5>
            <p>Feminina</p>
            <p>Dri-FIT</p>
            <h2>R$ 129,99
              R$ 249,99
              48% off
            ou 1x de R$ 129,99</h2>
          </div>
        </div>
      </div>
    

      <div class="col">
        <div class="card" style="width: 18rem;">
          <img src="img-projeto/brasil.avif" class="card-img-top" alt="...">
          <div class="card-body">
            <h5 class="card-title">Camiseta Nike Pro Dri-FIT Feminina</h5>
            <h5 class="card-title">Treino & Academia</h5>
            <p>Feminina</p>
            <p>Dri-FIT</p>
            <h2>R$ 169,99
              R$ 249,99
              32% off
          ou 1x de R$ 169,99</h2>
          </div>
        </div>
      </div>
    
      <div class="col">
        <div class="card" style="width: 18rem;">
          <img src="img-projeto/brasil.avif" class="card-img-top" alt="...">
          <div class="card-body">
            <h5 class="card-title">Camiseta Nike One Dri-FIT GRX Feminina</h5>
            <h5 class="card-title">Treino & Academia</h5>
            <p>Feminina</p>
            <p>Dri-FIT</p>
            <h2>R$ 179,99
              R$ 249,99
              28% off
            ou 1x de R$ 179,99</h2>
          </div>
        </div>
      </div>
    
      <div class="col">
        <div class="card" style="width: 18rem;">
          <img src="img-projeto/brasil.avif" class="card-img-top" alt="...">
          <div class="card-body">
            <h5 class="card-title">Camiseta Nike Dri-FIT Run Divsion Feminina</h5>
            <h5 class="card-title">Treino & Academia</h5>
            <p>Feminina</p>
            <p>Dri-FIT</p>
            <h2>R$ 299,99
              R$ 399,99
              25% off
          ou 3x de R$ 100,00</h2>
          </div>
        </div>
      </div>
    
      <div class="col">
        <div class="card" style="width: 18rem;">
          <img src="img-projeto/brasil.avif" class="card-img-top" alt="...">
          <div class="card-body">
            <h5 class="card-title">Camiseta Nike Dri-FIT Maternidade Feminina</h5>
            <h5 class="card-title">Treino & Academia</h5>
            <p>Feminina</p>
            <p>Dri-FIT</p>
            <h2>R$ 159,99
              R$ 219,99
              27% off
          ou 1x de R$ 159,99</h2>
          </div>
        </div>
      </div>
    
      <div class="col">
        <div class="card" style="width: 18rem;">
          <img src="img-projeto/brasil.avif" class="card-img-top" alt="...">
          <div class="card-body">
            <h5 class="card-title">Camiseta Nike Air Dri-FIT Feminina</h5>
            <h5 class="card-title">Treino & Academia</h5>
            <p>Feminina</p>
            <p>Dri-FIT</p>
            <h2>R$ 239,99
              R$ 279,99
              14% off
          ou 2x de R$ 120,00</h2>
          </div>
        </div>
      </div>
    
      <div class="col">
        <div class="card" style="width: 18rem;">
          <img src="img-projeto/brasil.avif" class="card-img-top" alt="...">
          <div class="card-body">
            <h5 class="card-title">Camiseta Nike Dri-FIT Run Division Feminina</h5>
            <h5 class="card-title">Treino & Academia</h5>
            <p>Feminina</p>
            <p>Dri-FIT</p>
            <h2>R$ 289,99
              R$ 379,99
              24% off
          ou 3x de R$ 96,66</h2>
          </div>
        </div>
      </div>
    
      <div class="col">
        <div class="card" style="width: 18rem;">
          <img src="img-projeto/brasil.avif" class="card-img-top" alt="...">
          <div class="card-body">
            <h5 class="card-title">Camiseta Nike Dri-FIT Hyverse Masculina</h5>
            <h5 class="card-title">Treino & Academia</h5>
            <p>Masculina</p>
            <p>Dri-FIT</p>
            <h2>R$ 139,99
              R$ 249,99
                44% off
            ou 1x de R$ 139,99</h2>
          </div>
        </div>
      </div>
    
      <div class="col">
        <div class="card" style="width: 18rem;">
          <img src="img-projeto/brasil.avif" class="card-img-top" alt="...">
          <div class="card-body">
            <h5 class="card-title">Camiseta Nike Dri-FIT Rise 365 Masculina</h5>
            <h5 class="card-title">Treino & Academia</h5>
            <p>Masculina</p>
            <p>Dri-FIT</p>
            <h2>R$ 269,99
              R$ 349,99
                23% off
          ou 2x de R$ 135,00</h2>
          </div>
        </div>
      </div>

      <div class="col">
        <div class="card" style="width: 18rem;">
          <img src="img-projeto/brasil.avif" class="card-img-top" alt="...">
          <div class="card-body">
            <h5 class="card-title">Camiseta Nike Pro Dri-FIT Masculina</h5>
            <h5 class="card-title">Treino & Academia</h5>
            <p>Masculina</p>
            <p>Dri-FIT</p>
          <h2>R$ 149,99
            R$ 199,99
            25% off
        ou 1x de R$ 149,99</h2>
          </div>
        </div>
      </div>

      <div class="col">
        <div class="card" style="width: 18rem;">
          <img src="img-projeto/brasil.avif" class="card-img-top" alt="...">
          <div class="card-body">
            <h5 class="card-title">Camiseta Nike Pro Dri-FIT Masculina</h5>
            <h5 class="card-title">Treino & Academia</h5>
            <p>Masculina</p>
            <p>Dri-FIT</p>
            <h2>R$ 129,99
              R$ 199,99
            35% off</h2>
          </div>
        </div>
      </div>

      <div class="col">
        <div class="card" style="width: 18rem;">
          <img src="img-projeto/brasil.avif" class="card-img-top" alt="...">
          <div class="card-body">
            <h5 class="card-title">Camiseta Nike Dri-FIT Academy 23 Masculina</h5>
            <h5 class="card-title">Treino & Academia</h5>
            <p>feminina</p>
            <p>Dri-FIT</p>
            <h2>R$ 69,99
              R$ 149,99
            53% off</h2>
          </div>
        </div>
      </div>

      <div class="col">
        <div class="card" style="width: 18rem;">
          <img src="img-projeto/brasil.avif" class="card-img-top" alt="...">
          <div class="card-body">
            <h5 class="card-title">Camiseta NikeCourt Dri-FIT Victory Masculina</h5>
            <h5 class="card-title">Treino & Academia</h5>
            <p>Masculina</p>
            <p>Dri-FIT</p>
            <h2>R$ 179,99
            R$ 279,99
          36% off
        ou 1x de R$ 179,99</h2>
          </div>
        </div>
      </div>

      <div class="col">
        <div class="card" style="width: 18rem;">
          <img src="img-projeto/brasil.avif" class="card-img-top" alt="...">
          <div class="card-body">
            <h5 class="card-title">Camiseta Nike Dri-FIT Body Shop 2 Masculina</h5>
            <h5 class="card-title">Treino & Academia</h5>
            <p>Masculina</p>
            <p>Dri-FIT</p>
            <h2>R$ 129,99
             R$ 279,99
           54% off
        ou 1x de R$ 129,99</h2>
          </div>
        </div>
      </div>

      <div class="col">
        <div class="card" style="width: 18rem;">
          <img src="img-projeto/brasil.avif" class="card-img-top" alt="...">
          <div class="card-body">
            <h5 class="card-title">Camiseta Nike Pro Dri-FIT Masculina</h5>
            <h5 class="card-title">Treino & Academia</h5>
            <p>Masculina</p>
            <p>Dri-FIT</p>
            <h2>R$ 79,99
           R$ 119,99
          33% off
       ou 1x de R$ 79,99</h2>
          </div>
        </div>
      </div>

      <div class="col">
        <div class="card" style="width: 18rem;">
          <img src="img-projeto/brasil.avif" class="card-img-top" alt="...">
          <div class="card-body">
            <h5 class="card-title">Camiseta Nike Dri-FIT Swoosh Masculina</h5>
            <h5 class="card-title">Treino & Academia</h5>
            <p>Masculina</p>
            <p>Dri-FIT</p>
            <h2>R$ 79,99
          R$ 119,99
        33% off
      ou 1x de R$ 79,99</h2>
          </div>
        </div>
      </div>

      <div class="col">
        <div class="card" style="width: 18rem;">
          <img src="img-projeto/brasil.avif" class="card-img-top" alt="...">
          <div class="card-body">
            <h5 class="card-title">Regata Nike Dri-FIT One Feminina</h5>
            <h5 class="card-title">Treino & Academia</h5>
            <p>Masculina</p>
            <p>Dri-FIT</p>
            <h2>R$ 69,99
          R$ 129,99
        46% off
      ou 1x de R$ 69,99</h2>
          </div>
        </div>
      </div>

      <div class="col">
        <div class="card" style="width: 18rem;">
          <img src="img-projeto/brasil.avif" class="card-img-top" alt="...">
          <div class="card-body">
            <h5 class="card-title">Camiseta Nike Dri-FIT Miler Top Masculina</h5>
            <h5 class="card-title">Treino & Academia</h5>
            <p>Masculina</p>
            <p>Dri-FIT</p>
            <h2>R$ 89,99
           R$ 149,99
         40% off
      ou 1x de R$ 89,99</h2>
          </div>
        </div>
      </div>




































  <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js" integrity="sha384-geWF76RCwLtnZ8qwWowPQNguL3RmwHVBC9FhGdlKrxdiJJigb/j/68SIy3Te4Bkz" crossorigin="anonymous"></script>
</body>

</html>